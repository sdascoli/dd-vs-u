...
def test(model, te_data, crit, task):
    with torch.no_grad():
        for (x,y) in te_data:
            out = model(x)
            test_loss = crit(out, y).mean().item()
...
if __name__ == '__main__':
    ...
    if args.task=='regression':
        with torch.no_grad():
            teacher = FullyConnected(width=args.teacher_width, n_layers=args.teacher_depth, in_dim=args.d, out_dim=args.n_classes)
        crit = nn.MSELoss()
    ...
    for width in widths:
        student = FullyConnected(width=width, n_layers=args.depth, in_dim=args.d, out_dim=args.n_classes)
